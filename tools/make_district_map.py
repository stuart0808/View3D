...
sys.path.insert(0, str(Path(__file__).resolve().parent))  # 直接 python tools/xxx.py 跑时也能 import 同目录的模块
...
CORE_S, CORE_X, CORE_Y = 0.4, 1800, 1260           # 街区示例的缩放和左上角（0.2 m/px × 0.4 = 0.5 m/px，正好对上）
CORE_W, CORE_H = int(3000 * CORE_S), int(2200 * CORE_S)  # 缩放后的核心区尺寸 1200×880px
...
def rounded_rect_path(x0, y0, x1, y1, r, n=10):
    ...
    pts = []
    for cx, cy, a0 in ((x1 - r, y0 + r, -90), (x1 - r, y1 - r, 0), (x0 + r, y1 - r, 90), (x0 + r, y0 + r, 180)):  # 四个圆心和起始角
        for k in range(n + 1):  # 每个圆角 n+1 个点，相邻圆角的端点重合无妨
...
def smooth(points, rounds=4):
    ...
    for _ in range(rounds):  # 每轮把每条边换成 1/4 和 3/4 处的两个点
        q = [p[0]]
...
def main():
    ...
    rng = np.random.default_rng(11)  # 固定种子，每次生成一样
    img = np.full((H, W, 3), (232, 234, 236), np.uint8)  # 浅灰底图
    ...
    for x in (330, 1500, 3300, 4470):                       # 主干路 24m（48px），南北向四条
        road_rect(x - 24, G0 - 24, 48, GY1 - G0 + 48)
    for y in (330, 3070):  # 东西向两条主干路（最北、最南）
        road_rect(G0 - 24, y - 24, GX1 - G0 + 48, 48)
    for x in (RING[0], RING[2]):                            # 环路下方的主干路 42m（中间 20m 在桥下）
        road_rect(x - 42, G0, 84, GY1 - G0)
    for y in (RING[1], RING[3]):  # 环路下面的东西向主干路
        road_rect(G0, y - 42, GX1 - G0, 84)
    art_y = CORE_Y + int(992 * CORE_S)                      # 横贯的主干路，和核心区里那条对齐（992 是街区示例里的 y）
    ...
    for y in (565, 2835):  # 东西向次干路
        road_rect(G0, y - 10, GX1 - G0, 20)
    for y in (1030, 2370):  # 环路内的东西向次干路，让开中间的核心区
        road_rect(G0, y - 10, 1500 - G0, 20)
        road_rect(3300, y - 10, GX1 - 3300, 20)
    # 核心区的街道向外接到格网上
    ax, bx, cx = CORE_X + 360, CORE_X + 680, CORE_X + 920  # 核心区支路 A / 次干路 B / 支路 C 的 x（900/1700/2300 × 0.4）
    road_rect(ax, RING[1], 19, CORE_Y + 60 - RING[1])  # A、B 向北接到环路下的主干路
    road_rect(bx, RING[1], 32, CORE_Y + 60 - RING[1])
    for x, w in ((ax, 19), (bx, 32), (cx, 19)):  # A、B、C 向南接出去
        road_rect(x, CORE_Y + CORE_H - 60, w, RING[3] - (CORE_Y + CORE_H - 60))
    road_rect(1500, CORE_Y + 248, CORE_X + 60 - 1500, 19)  # 核心区北侧支路向西接主干路
    road_rect(CORE_X + CORE_W - 60, CORE_Y + 624, 3300 - (CORE_X + CORE_W - 60), 19)  # 南侧支路向东接主干路

    # ------------------------------------------------------------------ 核心区: 街区示例缩小后嵌进来
    sx = lambda v: int(round(CORE_X + v * CORE_S))  # 街区坐标 → 城区坐标
    sy = lambda v: int(round(CORE_Y + v * CORE_S))
    sl = lambda v: max(1, int(round(v * CORE_S)))  # 长度缩放，至少 1px
    draw_demo(
        ...
        elevated=False,  # 街区自己的高架不要，城区有自己的快速路
    )
    core_rect = (CORE_X + 20, CORE_Y + 20, CORE_X + CORE_W - 20, CORE_Y + CORE_H - 20)  # 核心区范围，自动填楼时跳过

    # ------------------------------------------------------------------ 场馆
    reserved = np.zeros((H, W), np.uint8)  # 场馆及其广场占的地，自动填楼时跳过
    # 体育场: 核心区正北的大地块
    R(2560, 862, 690, 392, PLAZA)  # 站前广场
    cv2.ellipse(img, (2905, 1058), (285, 165), 0, 0, 360, VENUE, -1)  # 285×165px = 285×165m 的椭圆
    cv2.rectangle(reserved, (2512, 842), (3276, 1300), 255, -1)
    # 大剧院: 核心区西侧，临水
    R(1545, 1300, 215, 190, PLAZA)  # 剧院前广场
    cv2.ellipse(img, (1652, 1395), (88, 66), 0, 0, 360, VENUE, -1)  # 88×66m 的椭圆壳体
    R(1545, 1545, 215, 90, PARK)
    cv2.ellipse(img, (1652, 1590), (80, 28), 0, 0, 360, WATER, -1)
    cv2.rectangle(reserved, (1524, 1260), (1790, 1657), 255, -1)

    # ------------------------------------------------------------------ 铁路走廊（高架铁路下面留成绿廊，不盖楼）
    rail = smooth([(100, 180), (700, 480), (1400, 690), (2300, 715), (3100, 735), (3620, 920), (4050, 1500), (4330, 2300), (4700, 3260)])  # 从西北斜穿到东南，故意不正南正北
    corridor = np.zeros((H, W), np.uint8)
    cv2.polylines(corridor, [rail.round().astype(np.int32).reshape(-1, 1, 2)], False, 255, 70)  # 70px = 35m 宽的走廊

    # ------------------------------------------------------------------ 其余地块自动填充
    # free = 还能盖楼的地: 格网内 − 道路（含退让）− 核心区 − 场馆 − 环路弯角下方 − 铁路走廊
    free = np.zeros((H, W), np.uint8)
    cv2.rectangle(free, (G0, G0), (GX1, GY1), 255, -1)
    free[cv2.dilate(road, np.ones((25, 25), np.uint8)) > 0] = 0                      # 退让人行道: 路两侧各 12px = 6m
    cv2.rectangle(free, core_rect[:2], core_rect[2:], 0, -1)
    free[reserved > 0] = 0
    ring_path = rounded_rect_path(*RING, 170)  # 圆角半径 170px = 85m
    deck = np.zeros((H, W), np.uint8)
    cv2.polylines(deck, [ring_path.round().astype(np.int32).reshape(-1, 1, 2)], True, 255, 40)  # 桥面 40px = 20m 宽
    free[cv2.dilate(deck, np.ones((31, 31), np.uint8)) > 0] = 0                      # 环路弯角从街区上空切过，下面不盖楼
    green_under_rail = (corridor > 0) & (free > 0)  # 走廊里没被路 / 核心区占的部分
    img[green_under_rail] = GREEN
    free[corridor > 0] = 0  # 走廊不盖楼

    n, lab, stats, cent = cv2.connectedComponentsWithStats(free, connectivity=4)  # 每个连通块 = 一个地块
    used = np.zeros((H, W), np.uint8)  # 已放楼的像素
    ...
    def try_rect(block, x, y, w, h, col):
        ...
        if x < 0 or y < 0 or x + w >= W or y + h >= H:
            return False
        if (lab[y:y + h, x:x + w] != block).any() or used[y - 6:y + h + 6, x - 6:x + w + 6].any():  # 出了地块，或和别的楼太近
            return False
        R(x, y, w, h, col)
    ...
    for b in range(1, n):
        ...
        if (2179 < cxb < 2490 and 830 < cyb < 1300) or (2990 < cxb < 3290 and 1250 < cyb < 2150):
            kind = "residential"
        if kind == "park":  # 公园: 整块地内缩 7px 涂公园色，中间一个椭圆水池
        ...
        while y < y0 + bh - 30:
            ...
            while x < x0 + bw - 30:
                ...
                if t == "residential":
                    w, h, col = rng.integers(120, 180), 28, RESIDENTIAL  # 板楼: 60~90m 长、14m 进深
                elif t == "office":
                    w, h, col = rng.integers(70, 120), rng.integers(70, 110), MAGENTA  # 写字楼: 近方形
                else:
                    w, h, col = rng.integers(130, 230), rng.integers(80, 130), RED if rng.random() < 0.8 else ORANGE  # 商业: 大盒子，两成是单层
                if try_rect(b, x, y, w, h, col):
                    row_h = max(row_h, h)
                    if t == "residential" and rng.random() < 0.5:
                        try_rect(b, x, y + h + 10, w, 22, GREEN)  # 住宅楼前一条绿化带
                    x += w + rng.integers(28, 46)
                else:
                    x += 24
            y += row_h + (62 if kind == "residential" else rng.integers(34, 52))  # 住宅楼间距大（日照间距）

    # ------------------------------------------------------------------ 高架（画在最后，盖在地面道路上）
    exp_y = art_y + 43  # 横贯的高架快速路压在主干路正中
    cv2.rectangle(img, (60, exp_y - 20), (W - 60, exp_y + 20), ELEVATED, -1)  # 横贯的快速路 20m 宽，两端通到图外
    cv2.polylines(img, [ring_path.round().astype(np.int32).reshape(-1, 1, 2)], True, ELEVATED, 40)  # 环形高架

    # ------------------------------------------------------------------ sidecar: 轨道交通 + 场馆 + 核心区
    P = lambda pts: [[round(float(x), 1), round(float(y), 1)] for x, y in pts]  # 像素坐标保留 1 位小数
    mx = bx + 16  # 南北线走核心区次干路 B 的正下方（B 宽 32px，取中线）
    sidecar = {
        "mpp": MPP,
        "activeRegion": ACTIVE,
        "venues": [
    ...
    cv2.imencode(".png", img)[1].tofile(str(out_dir / "district_marked.png"))  # tofile 支持中文路径
