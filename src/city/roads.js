 ...
 *   单行路: 整幅都是同向车道；环道按宽度推最多 2 条（再多内圈弧段太短）
 *   laneCount > 0（场景编辑器画路时指定的每方向车道数）: 直接用它（1~4），车道宽 = 可用宽度 / n
 */
export function laneLayout(width, oneway = false, median = 0, laneCount = 0) {
  const given = laneCount > 0 ? Math.min(4, Math.round(laneCount)) : 0 // 指定了车道数就不按路宽推
  if (oneway) {
    const n = given || Math.max(1, Math.min(2, Math.floor(width / 3.5))) // 环道最多两圈车道，再多内圈弧段太短
    const laneW = width / n
    return { n, laneW, inner: 0, offsets: Array.from({ length: n }, (_, k) => -width / 2 + (k + 0.5) * laneW) }
  }
  const inner = median > 0 ? median + 0.4 : 0
  const n = given || Math.max(1, Math.min(4, Math.floor((width / 2 - inner) / 3.3)))
...
export function hasMedian(width, oneway = false, laneCount = 0) {
  return !oneway && laneLayout(width, false, 0, laneCount).n >= 3 // 指定车道数时按指定的算
}
